...
歌单列表页不需要后退按钮，无菜单内容，FAB 为**新增歌单**。支持多选。多选后**后退按钮**为**取消选择**，菜单内容为**删除歌单**（由文件夹建立的歌单是无法被删除的）。
...
歌单歌曲列表页，**后退按钮**为**返回歌单列表**，菜单内容为**删除歌单**（若为文件夹建立的歌单则无菜单内容）FAB 为**播放歌单**。支持多选。多选后**后退按钮**为**取消多选**，菜单内容为**全选**、**加入播放队列**、**加入到其他歌单**、**从歌单中移除**（若为文件夹建立的歌单则无此项）。
